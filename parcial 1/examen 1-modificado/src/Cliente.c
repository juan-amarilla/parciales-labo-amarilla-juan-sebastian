...
int altaCliente(eCliente listaCliente[], int tam, eLocalidad listaLocalidad[], int tamLocal, char cadena[], int* id)
{
	...
	for(i=0;i<tam;i++)
	{
		...
		if(buscarLibre(listaCliente[i].isEmpty) != OCUPADO && id != NULL)
		{
			*id += 1;
			listaCliente[i] = pedirCliente(listaCliente[i], cadena, id, listaCliente);
			pedirUnaLocalidad(listaLocalidad, tamLocal,  &listaCliente[i].idLocalidad);
			retorno = 1;
			break;
...
int modificarCliente(eCliente listaCliente[], int tam, eLocalidad listaLocalidad[], int tamLocal, char cadena[])
{
	...
	if(seguro == 1)
	{
		retorno = modificarUnDatoDeCliente(listaCliente, tam, listaLocalidad, tamLocal, cadena);
